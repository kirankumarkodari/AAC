...
begin
  ...
  resIdx := 0;
  SetLength(pallertBuffer, 257, 3); // PALLETRING BUFFER

  for heiIdx := 0 to 256 do
  begin
    for widIdx := 0 to 2 do
    begin
      pallertBuffer[heiIdx, widIdx] := '0XFF';
    end;
  end;

  try
    While resIdx <= Length(resStr) - 4 do
    begin
      rptcolorcount := 1;
    ...
    begin
      ShowMessage(
        'Exception  Image Has More than 256 Colors OutPut Will Be Shown For 256 Colors');
    end;
  ...
  WriteLn(myFile,'// Pallert Buffer With BGR values of each pixel');
